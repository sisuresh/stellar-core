                                 ...
                                 LedgerTxnConsistency cons) noexcept
{
    ...
    try
    {
        while ((bool)iter)
        {
            // Collect extension keys so we can apply them later.
            if (iter.key().type() == InternalLedgerEntryType::LEDGER_ENTRY &&
        ...
        auto updateExpiration = [&](LedgerEntryType type) {
            UnorderedSet<LedgerKey> dataEntryKeys;
            for (auto const& kvp : expirationExtensions)
            {
                if (kvp.first.type() == type)
                {
                    dataEntryKeys.emplace(
                        dataEntryKeyFromExpirationExtensionKey(kvp.first));
                }
            }

            if (dataEntryKeys.empty())
            {
                return;
            }
            ...
            auto contractDataDataEntries =
                type == CONTRACT_CODE ? bulkLoadContractCode(dataEntryKeys)
                                      : bulkLoadContractData(dataEntryKeys);
            for (auto& kvp : contractDataDataEntries)
            {
                ...
                releaseAssertOrThrow(it != expirationExtensions.end());
        ...
        updateExpiration(CONTRACT_CODE);
        updateExpiration(CONTRACT_DATA);
