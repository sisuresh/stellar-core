...
{
    std::vector<TransactionResultPair> innerResults;
    innerResults.reserve(actualResults.results.size());
    for (auto const& result : actualResults.results)
    {
        ...
        innerResults.emplace_back(trp);
    }

    assert(innerResults.size() == expectedResults.size());
    for (size_t i = 0; i < innerResults.size(); ++i)
    {
        innerResults[i].result.feeCharged =
            expectedResults[i].result.feeCharged;
        if (!(innerResults[i] == expectedResults[i]))
        {
            CLOG(ERROR, "History")
                << fmt::format("Expected operation result {} but got {}",
                               xdr::xdr_to_string(expectedResults[i]),
                               xdr::xdr_to_string(innerResults[i]));
        }
    }
}
}
