...
class GenerateBucketsWork;
class Bucket;
class TmpDir;

/*
 * SimulateApplyBucketsWork will retrieve the current bucketlist, transform it
 * by creating new Ledger Entries to mimic existing ones, and apply newly
 * created buckets
 * */
class SimulateApplyBucketsWork : public Work
{
    Application& mApp;
    std::shared_ptr<GetHistoryArchiveStateWork> mGetState;
    std::shared_ptr<GenerateBucketsWork> mGenerateBucketsWork;
