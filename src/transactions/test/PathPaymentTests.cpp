...
{
    auto pathPaymentTests = [](bool emitEvents) {
        ...
        SECTION("transact more than INT64_MAX in a path payment")
        {
            ...
            for_versions_from(10, *app, [&] {
                SECTION("issue more than INT64_MAX")
                {
                    ...
                    // a2 is buying cur2. This is necessary so a1 can get rid of
                    // the first INT64_MAX cur2 during the path payment so it
                    // can cross the issuers second offer to get another
                    // INT64_MAX cur2
                    a2.manageOffer(0, cur1, cur2, Price{1, 1}, INT64_MAX,
                                   MANAGE_OFFER_CREATED);

                    // offer to issue cur2 for cur1
                    gateway.createPassiveOffer(cur2, cur1, Price{1, 1},
                                               INT64_MAX, MANAGE_OFFER_CREATED);
                ...
                SECTION("burn more than INT64_MAX")
                {
                    ...
                    // a2 is buying idr for cur2. This is necessary so a1 can
                    // get more cur2 to cross the issuers second offer and burn
                    // another INT64_MAX cur2.
                    ...
                    // CUR2 -> IDR -> CUR2 -> CUR1
                    a1.pay(gateway, cur2, INT64_MAX, cur1, INT64_MAX,
                           {idr, cur2});
        ...
        SECTION("path payment send currency invalid")
        {
            ...
            gateway.pay(source, idr, 10);
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(source.pay(destination, makeInvalidAsset(),
                                             10, idr, 10, {}),
                                  ex_PATH_PAYMENT_STRICT_RECEIVE_MALFORMED);
                // clang-format off
                market.requireBalances(
        ...
        SECTION("path payment destination currency invalid")
        {
            ...
            gateway.pay(source, idr, 10);
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(source.pay(destination, idr, 10,
                                             makeInvalidAsset(), 10, {}),
                                  ex_PATH_PAYMENT_STRICT_RECEIVE_MALFORMED);
                // clang-format off
                market.requireBalances(
        ...
        SECTION("path payment destination path currency invalid")
        {
            ...
            gateway.pay(source, idr, 10);
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(source.pay(destination, idr, 10, idr, 10,
                                             {makeInvalidAsset()}),
                                  ex_PATH_PAYMENT_STRICT_RECEIVE_MALFORMED);
                // clang-format off
                market.requireBalances(
        ...
        SECTION("path payment XLM with not enough funds")
        {
            ...
            auto minimumAccount = root->create("minimum-account",
                                               minBalanceNoTx + 2 * txfee + 20);
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(minimumAccount.pay(*root, xlm, txfee + 21,
                                                     xlm, txfee + 21, {}),
        ...
        SECTION("path payment asset with not enough funds")
        {
            ...
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(
                    minimumAccount.pay(gateway, idr, 11, idr, 11, {}),
                    ex_PATH_PAYMENT_STRICT_RECEIVE_UNDERFUNDED);
                // clang-format off
                market.requireBalances(
        ...
        SECTION("path payment source does not have trustline")
        {
            ...
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(
                    noSourceTrust.pay(gateway, idr, 1, idr, 1, {}),
        ...
        SECTION(
            "path payment destination is issuer and does not exists for simple "
            "paths")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance1);
                ...
                "complex paths")
        {
            ...
            for_all_versions(*app, [&] {
                gateway.merge(*root);
                REQUIRE_THROWS_AS(
                    source.pay(gateway, idr, 10, usd, 10, {}),
                    ex_PATH_PAYMENT_STRICT_RECEIVE_NO_DESTINATION);
                // clang-format off
                market.requireBalances(
                    {{source, {{xlm, minBalance1 - 2 * txfee}, {idr, 10}, {usd, 0}}}});
            ...
            SECTION("allow maintain liabilities")
            {
                for_versions_from(13, *app, [&] {
                    gateway.allowMaintainLiabilities(
                        idr, noAuthorizedDestinationTrust);
                    destNotAuthorized();
                });
            }
        }
        ...
        {
            // look at the SECTION "path payment uses all offers in a loop" for
            // a successful path payment test with no issuers
        ...
        SECTION(
            "path payment does not cross own offer if better is available for "
            "first exchange")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance3);
            ...
            for_all_versions(*app, [&] {
                ...
                market.requireChanges(
                    ...
                    [&] {
                        actual = source
                                     .pay(destination, cur1, 10, cur4, 10,
                                          {cur1, cur2, cur3, cur4})
                                     .success()
                                     .offers;
                    });
        ...
        SECTION("path payment over send max with real path")
        {
            ...
            for_all_versions(*app, [&] {
                REQUIRE_THROWS_AS(market.requireChanges(
                                      {},
                                      [&] {
                                          source.pay(destination, cur1, 10,
                                                     cur4, 10,
                                                     {cur1, cur2, cur3, cur4});
                                      }),
                                  ex_PATH_PAYMENT_STRICT_RECEIVE_OVER_SENDMAX);
        ...
        auto usesWholeBestOffer = [&](bool
            ...
            SECTION("path payment uses whole best offer for first exchange")
            {
                ...
                auto o1a = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm12a,
                                           {cur2, cur1, Price{2, 1}, 10});
                });
                auto o1b = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm12b,
                                           {cur2, cur1, Price{2, 1}, 40});
                });
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm23, {cur3, cur2, Price{2, 1}, 20});
                });
            ...
            SECTION("path payment uses whole best offer for second exchange")
            {
                ...
                auto o2a = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm23a,
                                           {cur3, cur2, Price{2, 1}, 15});
                });
                auto o2b = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm23b,
                                           {cur3, cur2, Price{2, 1}, 10});
                });
                auto o3 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm34, {cur4, cur3, Price{2, 1}, 10});
                });
            ...
            SECTION("path payment uses whole best offer for last exchange")
            {
                ...
                auto o3b = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm34b,
                                           {cur4, cur3, Price{2, 1}, 10});
                });
        ...
        SECTION(
            "path payment full trust line for buying asset for offer for last "
            "exchange")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance4);
                ...
                "first exchange")
        {
            ...
            // impossible to have an offer with excess selling liabilities. This
            // can be verified from the following tests:
            //   - Cannot use PaymentOp or PathPaymentOp to reduce balance below
            //     selling liabilities (tested in "payment"/"pathpayment"
            ...
            //     lead to excess liabilities" subsection "* selling
            //     liabilities")
        }
        ...
        SECTION(
            "path payment 1 in trust line for selling asset for offer for last "
            "exchange")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance4);
        ...
        SECTION(
            "path payment 1 left in trust line for buying asset for offer for "
            "first exchange")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance4);
            ...
            //     buying liabilities above limit (tested in
        ...
        SECTION("path payment takes all offers, multiple offers per exchange")
        {
            ...
            for_versions(3, 9, *app, [&] {
                // Some of these offers are invalid starting in version 10, so
                // we can't run this test in that case.
                auto o1a = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm12, {cur2, cur1, Price{3, 2}, 10});
                });
                auto o1b = market.requireChangesWithOffer({}, [&] {
                ...
                market.requireChanges(
                    ...
                    [&] {
                        actual = source
                                     .pay(destination, cur1, 45, cur4, 10,
                                          {cur1, cur2, cur3, cur4})
                                     .success()
                                     .offers;
                    });
        ...
        SECTION(
            "path payment takes all offers, multiple offers per exchange V10")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance4);
            auto destination = root->create("destination", minBalance1);
            ...
            for_versions_from(10, *app, [&] {
                ...
                market.requireChanges(
                    ...
                    [&] {
                        actual = source
                                     .pay(destination, cur1, 59, cur4, 16,
                                          {cur1, cur2, cur3, cur4})
                                     .success()
                                     .offers;
                    });
        ...
        SECTION("path payment takes best offers, multiple offers per exchange")
        {
            ...
            for_versions(3, 9, *app, [&] {
                ...
                market.requireChanges(
                    ...
                    [&] {
                        actual = source
                                     .pay(destination, cur1, 29, cur4, 8,
                                          {cur1, cur2, cur3, cur4})
                                     .success()
                                     .offers;
                    });
        ...
        SECTION(
            "path payment takes best offers, multiple offers per exchange V10")
        {
            auto market = TestMarket{*app};
            auto source = root->create("source", minBalance4);
            auto destination = root->create("destination", minBalance1);
            ...
            for_versions_from(10, *app, [&] {
                ...
                market.requireChanges(
                    ...
                    [&] {
                        actual = source
                                     .pay(destination, cur1, 51, cur4, 15,
                                          {cur1, cur2, cur3, cur4})
                                     .success()
                                     .offers;
                    });
                ...
                "offers trail")
        {
            ...
            auto initialBalance =
                app->getLedgerManager().getLastMinBalance(10) + txfee * 10 +
                1000000000;
                ...
                auto idrCurExpensiveOffer =
                    market.requireChangesWithOffer({}, [&] {
                        return market.addOffer(
                            mm, {idr, usd, Price{6, 12}, offerSize});
                    });
                auto usdCurOffer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(
                        mm, {usd, xlm, Price{1, 2}, 2 * offerSize});
                });
        ...
        SECTION("path payment with cycle")
        {
            for_all_versions(*app, [&] {
                ...
                // accounts, offers and assets - it greatly simplified index
                // calculation in the code.

                auto market = TestMarket{*app};
                // amount of money that 'destination' account will receive
                auto paymentAmount = int64_t{100000000};
                // amount of money in offer required to pass - needs 8x of
                // payment for anti-arbitrage case
                ...
                auto validateAccountAsset = [&](const TestAccount& account,
                    ...
                    {
                        REQUIRE(
                            account.loadTrustLine(assets[assetIndex]).balance ==
                ...
                auto validateOffer = [&](const TestAccount& account,
                    ...
                    LedgerTxn ltx(app->getLedgerTxnRoot());
                    auto offer = stellar::loadOffer(ltx, account.getPublicKey(),
                                                    offerId);
                    ...
                    SECTION(name)
                    {
                        ...
                        for (int i = 0; i < pathSize; i++)
                        {
                                    ...
                                    assetPathToString(assets) + ")")
                            {
                                ...
                                {
                                    // it is done from end of path to begin of
                                    // path
                                    ...
                                    validateOffer(
                                        ...
                                        -paymentAmount * sellerMultiplier);
                                    // bought asset
                                    validateAccountAsset(
                ...
                // cycle with every asset on path costing twice as
                // much as previous - 8 times loss - acceptable (but not wise to
                // do)
        ...
        SECTION("path payment rounding")
        {
            ...
            SECTION("exchangeV10 recalculate sheepValue: 1 offer")
            {
                ...
                auto o1 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm,
                                           {cur2, cur1, Price{1, 1000}, 2000});
                });

                for_versions_from(10, *app, [&] {
                    market.requireChanges({{o1.key, OfferState::DELETED}}, [&] {
                        source.pay(destination, cur1, 2, cur2, 1001,
                                   {cur1, cur2});
                    });
        ...
        SECTION("liabilities")
        {
            SECTION("cannot pay balance below selling liabilities")
            {
                ...
                auto offer = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(source,
                                           {cur1, xlm, Price{1, 1}, 50});
                });
                auto o2 = market.requireChangesWithOffer({}, [&] {
                    return market.addOffer(mm12,
                                           {cur2, cur1, Price{1, 1}, 100});
                });
                ...
                for_versions_from(10, *app, [&] {
                    REQUIRE_THROWS_AS(
                        source.pay(destination, cur1, 51, cur2, 51,
                                   {cur1, cur2}),
                        ex_PATH_PAYMENT_STRICT_RECEIVE_UNDERFUNDED);
            ...
            SECTION(
                "cannot receive such that balance + buying liabilities exceeds"
                " limit")
            {
                TestMarket market(*app);
                auto source = root->create("source", minBalance2);
                ...
                for_versions_from(10, *app, [&] {
                    REQUIRE_THROWS_AS(source.pay(destination, cur1, 51, cur2,
                                                 51, {cur1, cur2}),
                                      ex_PATH_PAYMENT_STRICT_RECEIVE_LINE_FULL);
                    source.pay(destination, cur1, 50, cur2, 50, {cur1, cur2});
                });
            ...
            SECTION("offer sponsor is source or dest account")
            {
                auto runTest = [&](Asset const& selling, Asset const& buying,
                    ...
                    SECTION("cross one offer partially")
                    {
                        ...
                        LedgerTxn ltx(app->getLedgerTxnRoot());
                        checkSponsorship(ltx, offerKey(o1.sellerID, o1.offerID),
                                         1, &b.getPublicKey());
                        checkSponsorship(ltx, offerKey(o2.sellerID, o2.offerID),
                                         1, &b.getPublicKey());
                        checkSponsorship(ltx, offerKey(o3.sellerID, o3.offerID),
                                         1, &b.getPublicKey());
        ...
        SECTION(
            "crossed offers release sponsorships allowing payment to succeed "
            "for source")
        {
    ...
    SECTION("with events")
    {
        pathPaymentTests(true);
    }

    SECTION("without events")
    {
        pathPaymentTests(false);
    }
}
