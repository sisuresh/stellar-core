...
{
    auto const& opRes = tx->getResult().result.results()[i];
    return opRes.tr().confirmAndClearSponsorResult().code();
}
    ...
    SECTION("not sponsored")
    {
        for_versions_from(14, *app, [&] {
            ...
            REQUIRE(getConfirmAndClearSponsorResultCode(tx, 0) ==
                    CONFIRM_AND_CLEAR_SPONSOR_NOT_SPONSORED);
        });
    }

    // CONFIRM_AND_CLEAR_SPONSOR success tested in SponsorFutureReservesTests
}
