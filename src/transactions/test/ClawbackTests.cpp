...
TEST_CASE("clawback", "[tx][clawback]")
{
    ...
    for_versions_from(16, *app, [&] {
        ...
        SECTION("allow trust")
        {
            ...
            {
                REQUIRE_THROWS_AS(gateway.allowTrust(
                                      idr, a1, TRUSTLINE_CLAWBACK_ENABLED_FLAG),
                                  ex_ALLOW_TRUST_MALFORMED);

                // show that TRUSTLINE_CLAWBACK_ENABLED_FLAG was the reason the
                // above op failed
                gateway.allowTrust(idr, a1, 0);
            }
        }
        ...
        SECTION("errors")
        {
            SECTION("set options")
            {
                // can't clear revocable if AUTH_CLAWBACK_ENABLED_FLAG is set
                REQUIRE_THROWS_AS(
