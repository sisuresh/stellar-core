          ...
          "[tx][soroban][archival]")
{
    ...
    auto initialTTL = test.getTTL(lk);
    ...
    SECTION("multiple readonly ttl bumps same stage")
    {
        ...
        // Verify liveUntilLedgerSeq in meta for each transaction in same stage
        auto& lm = test.getApp().getLedgerManager();
        auto opMeta0 =
            lm.getLastClosedLedgerTxMeta().at(0).getLedgerEntryChangesAtOp(0);
    ...
    SECTION("readonly ttl bumps with mixed stages")
    {
        ...
        // Verify liveUntilLedgerSeq in meta for mixed stages
        auto& lm = test.getApp().getLedgerManager();

        // First stage: tx0 and tx1 in same stage
        auto opMeta0 =
            lm.getLastClosedLedgerTxMeta().at(0).getLedgerEntryChangesAtOp(0);
        REQUIRE(opMeta0.at(0).state().data.ttl().liveUntilLedgerSeq ==
                startingTTL);
        ...
        // Third stage: tx3 alone
        auto opMeta3 =
            lm.getLastClosedLedgerTxMeta().at(3).getLedgerEntryChangesAtOp(0);
        REQUIRE(opMeta3.at(0).state().data.ttl().liveUntilLedgerSeq ==
                test.getLCLSeq() + 300);
        REQUIRE(opMeta3.at(1).updated().data.ttl().liveUntilLedgerSeq ==
                test.getLCLSeq() + 350);
