                                      ...
                                      LedgerTxnDelta const& ltxDelta)
{
    ...
    auto opSourceStrKey = KeyUtils::toStrKey(opSource);
    if (operation.body.type() == ACCOUNT_MERGE)
    {
        CLOG_ERROR(Tx, "{} minted {}", opSourceStrKey, deltaBalances);
    }
    else if (operation.body.type() == PAYMENT)
    {
        CLOG_ERROR(Tx, "{} minted {}", opSourceStrKey, deltaBalances);
    }
    else if (operation.body.type() == PATH_PAYMENT_STRICT_RECEIVE)
    {
        CLOG_ERROR(Tx, "{} burned {}", opSourceStrKey, deltaBalances);
    }
    else
    {
