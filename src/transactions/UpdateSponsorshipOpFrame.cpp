...
UpdateSponsorshipOpFrame::UpdateSponsorshipOpFrame(Operation const& op,
    ...
    , mUpdateSponsorshipOp(mOperation.body.updateSponsorshipOp())
{
}

ThresholdLevel
UpdateSponsorshipOpFrame::getThresholdLevel() const
{
...
UpdateSponsorshipOpFrame::updateLedgerEntrySponsorship(AbstractLedgerTxn& ltx)
{
    ...
    if (!ltxe)
    {
        innerResult().code(UPDATE_SPONSORSHIP_DOES_NOT_EXIST);
        return false;
    }
    ...
    if (le.ext.v() == 1)
    {
        ...
        if (extV1.sponsoringID)
        {
            if (!(*extV1.sponsoringID == getSourceID()))
            {
                ...
                // source account
                innerResult().code(UPDATE_SPONSORSHIP_NOT_SPONSOR);
                return false;
            }
    ...
    // SponsoringFutureReserves(A) = B      -> Sponsor(le) = B TODO(jonjove)
...
UpdateSponsorshipOpFrame::updateSignerSponsorship(AbstractLedgerTxn& ltx)
{
    ...
    // TODO(jonjove): This is also done in SetOptions, refactor
    auto it = std::find_if(
    ...
    // SponsoringFutureReserves(A) = B      -> Sponsor(it) = B TODO(jonjove)
